...
import { openProject, getClipServerToken } from "@/commands/fs"
import { getLastProject, getRecentProjects, saveLastProject, loadLlmConfig, loadLanguage, loadSearchApiConfig, loadEmbeddingConfig, loadAppTheme, loadFontScale } from "@/lib/project-store"
...
import { startRawWatcher, stopRawWatcher } from "@/lib/raw-watcher"
...
function App() {
  ...
  useEffect(() => {
    ...
    document.documentElement.style.setProperty("--app-font-scale", String(useWikiStore.getState().fontScale))
  }, [])
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        const savedFontScale = await loadFontScale()
        if (!cancelled && savedFontScale) {
          useWikiStore.getState().setFontScale(savedFontScale)
        }
