...
const TYPE_OPTIONS: ColumnType[] = [
  ...
  "transferFee", "totalCost", "modeTag", "certaintyTag",
  "plannedTrade", "exitCondition", "errorTag", "errorCost",
  "note", "ignore",
]
...
export function TradeImportPreview({
  ...
  const handleConfirm = () => {
    const result: Record<ColumnType, number | null> = {
      ...
      modeTag: null, certaintyTag: null, plannedTrade: null,
      exitCondition: null, errorTag: null, errorCost: null, note: null,
      ignore: null,
    }
    for (const [colIdx, type] of Object.entries(mapping)) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onCancel()}>
      <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col overflow-hidden p-0">
        ...
        <div className="mx-6 shrink-0 rounded-md border bg-muted/40 p-3 text-xs text-muted-foreground">
          可选交易大脑字段：模式、确定性、计划内、退出条件、错误标签、错误成本、备注。券商原始交割单没有这些列也可以导入，后续可在 Markdown 里补标。
        </div>
