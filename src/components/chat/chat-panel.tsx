...
import { PROVIDER_DEFINITIONS, applyProviderDefaults } from "@/lib/llm-catalog"
...
function ChatModelBar() {
  const llmConfig = useWikiStore((s) => s.llmConfig)
  const setLlmConfig = useWikiStore((s) => s.setLlmConfig)

  async function handleProviderChange(provider: typeof llmConfig.provider) {
    const next = applyProviderDefaults(llmConfig, provider)
    setLlmConfig(next)
    await saveLlmConfig(next)
  }

  async function handleModelChange(model: string) {
    const next = { ...llmConfig, model }
    setLlmConfig(next)
    await saveLlmConfig(next)
  }
    ...
    <div className="border-b bg-background/80 px-3 py-2 backdrop-blur">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <select
          ...
          onChange={(e) => void handleProviderChange(e.target.value as typeof llmConfig.provider)}
        ...
        <Input
          ...
          className="h-8 min-w-[220px] max-w-[360px] text-xs"
        />
      </div>
    </div>
  )
