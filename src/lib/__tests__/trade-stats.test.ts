...
describe("Trade Stats", () => {
  ...
  it("parses B/S direction exports", () => {
    const markdown = `# 2026-05-12 交割单

| 代码 | 名称 | 方向 | 类型 | 数量 | 价格 | 金额 | 手续费 |
|------|------|------|------|------|------|------|------|
| 000001 | 平安银行 | B | 买入 | 1000 | 10.50 | 10500 | 5.25 |
| 000001 | 平安银行 | S | 卖出 | 500 | 10.80 | 5400 | 2.70 |
`
    const stats = parseTradeMarkdown("2026-05-12", markdown)
...
`
  ...
  it("limits unknown cost warning to the selected date range", () => {
    const outOfRangeSellMd = `# 2025-04-30
| 浠ｇ爜 | 鍚嶇О | 鏂瑰悜 | 绫诲瀷 | 鏁伴噺 | 浠锋牸 | 閲戦 | 鎵嬬画璐?|
|------|------|------|------|------|------|------|------|
| 000001 | TestBank | S | 鍗栧嚭 | 1000 | 10.80 | 10800 | 5.40 |
`
    const inRangeBuySellMd = `# 2025-05-06
| 浠ｇ爜 | 鍚嶇О | 鏂瑰悜 | 绫诲瀷 | 鏁伴噺 | 浠锋牸 | 閲戦 | 鎵嬬画璐?|
|------|------|------|------|------|------|------|------|
| 000002 | TestStock | B | 涔板叆 | 1000 | 10.00 | 10000 | 5.00 |
| 000002 | TestStock | S | 鍗栧嚭 | 1000 | 11.00 | 11000 | 5.50 |
`
    const day1 = parseTradeMarkdown("2025-04-30", outOfRangeSellMd)
    const day2 = parseTradeMarkdown("2025-05-06", inRangeBuySellMd)
    const { overall } = computeDashboardStats([day1, day2], { start: "2025-05-01", end: "2025-05-31" })
