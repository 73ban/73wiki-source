...
const HEADER_MAP: Record<keyof TradeRecord, string[]> = {
  ...
  modeTag: ["模式", "交易模式", "买点", "mode", "mode_tag", "trade_mode", "strategy"],
  certaintyTag: ["确定性", "确定性等级", "等级", "certainty", "certainty_tag", "certainty_level", "level"],
  plannedTrade: ["计划", "计划内", "是否计划内", "作战室", "planned", "planned_trade", "plan"],
  exitCondition: ["退出条件", "止损条件", "卖出条件", "exit", "exit_condition", "exit_plan"],
  errorTag: ["错误", "错误标签", "纪律问题", "error", "error_tag", "mistake"],
  errorCost: ["错误成本", "纪律成本", "error_cost", "mistake_cost", "cost_of_error"],
  note: ["备注", "说明", "理由", "note", "notes", "remark", "remarks", "comment"],
}
...
function parseOptionalString(value: unknown): string | undefined {
  const text = stripExcelFormula(String(value ?? "").trim())
  return text ? text : undefined
}

function parseOptionalBoolean(value: unknown): boolean | undefined {
  const text = stripExcelFormula(String(value ?? "").trim()).toLowerCase()
  if (!text) return undefined
  if (["1", "true", "yes", "y", "计划内", "是", "作战室", "主计划", "备选计划", "planned"].includes(text)) return true
  if (["0", "false", "no", "n", "计划外", "非计划", "否", "冲动", "模式外", "unplanned"].includes(text)) return false
  if (text.includes("计划外") || text.includes("非计划") || text.includes("冲动") || text.includes("模式外")) return false
  if (text.includes("计划内") || text.includes("作战室") || text.includes("主计划") || text.includes("备选计划")) return true
  return undefined
}
...
export function parseTradeRecords(rows: unknown[][], fallbackDate?: string): TradeRecord[] {
  ...
  for (let i = rowIndex + 1; i < rows.length; i++) {
    ...
    records.push({
      ...
      transferFee,
      totalCost,
      modeTag: indices.modeTag != null ? parseOptionalString(row[indices.modeTag]) : undefined,
      certaintyTag: indices.certaintyTag != null ? parseOptionalString(row[indices.certaintyTag]) : undefined,
      plannedTrade: indices.plannedTrade != null ? parseOptionalBoolean(row[indices.plannedTrade]) : undefined,
      exitCondition: indices.exitCondition != null ? parseOptionalString(row[indices.exitCondition]) : undefined,
      errorTag: indices.errorTag != null ? parseOptionalString(row[indices.errorTag]) : undefined,
      errorCost: indices.errorCost != null ? Math.abs(parseNumber(row[indices.errorCost])) : undefined,
      note: indices.note != null ? parseOptionalString(row[indices.note]) : undefined,
    })
  }
...
function scoreColumn(values: unknown[], header: string): { type: ColumnType; confidence: number }[] {
  ...
  const headerHints: Partial<Record<ColumnType, string[]>> = {
    ...
    modeTag: ["模式", "交易模式", "买点", "mode", "strategy"],
    certaintyTag: ["确定性", "等级", "certainty", "level"],
    plannedTrade: ["计划", "作战室", "planned", "plan"],
    exitCondition: ["退出", "止损", "卖出条件", "exit"],
    errorTag: ["错误", "纪律", "问题", "error", "mistake"],
    errorCost: ["错误成本", "纪律成本", "error_cost", "mistake_cost"],
    note: ["备注", "说明", "理由", "note", "remark", "comment"],
  }
  ...
  for (const [type, hints] of Object.entries(headerHints)) {
    for (const hint of hints) {
      if (normalizedHeader.includes(hint.toLowerCase())) {
        headerBonus[type as ColumnType] = ["modeTag", "certaintyTag", "plannedTrade", "exitCondition", "errorTag", "errorCost", "note"].includes(type) ? 0.95 : 0.3
        break
      }
    }
  }

  const optionalHeaderType = (["modeTag", "certaintyTag", "plannedTrade", "exitCondition", "errorTag", "errorCost", "note"] as ColumnType[])
    .find((type) => headerBonus[type])
  if (optionalHeaderType) {
    return [{ type: optionalHeaderType, confidence: 0.95 }]
  }
  ...
  if (headerBonus.modeTag) {
    results.push({ type: "modeTag", confidence: headerBonus.modeTag })
  }
  if (headerBonus.certaintyTag) {
    results.push({ type: "certaintyTag", confidence: headerBonus.certaintyTag })
  }
  if (headerBonus.plannedTrade) {
    results.push({ type: "plannedTrade", confidence: headerBonus.plannedTrade })
  }
  if (headerBonus.exitCondition) {
    results.push({ type: "exitCondition", confidence: headerBonus.exitCondition })
  }
  if (headerBonus.errorTag) {
    results.push({ type: "errorTag", confidence: headerBonus.errorTag })
  }
  if (feeCount / total > 0.5 && headerBonus.errorCost) {
    results.push({ type: "errorCost", confidence: Math.min(1, feeCount / total + headerBonus.errorCost) })
  }
  if (headerBonus.note) {
    results.push({ type: "note", confidence: headerBonus.note })
  }
...
export function parseTradeRecordsWithMapping(
  ...
  for (let i = headerRowIndex + 1; i < rows.length; i++) {
    ...
    records.push({
      ...
      transferFee,
      totalCost,
      modeTag: mapping.modeTag != null ? parseOptionalString(row[mapping.modeTag]) : undefined,
      certaintyTag: mapping.certaintyTag != null ? parseOptionalString(row[mapping.certaintyTag]) : undefined,
      plannedTrade: mapping.plannedTrade != null ? parseOptionalBoolean(row[mapping.plannedTrade]) : undefined,
      exitCondition: mapping.exitCondition != null ? parseOptionalString(row[mapping.exitCondition]) : undefined,
      errorTag: mapping.errorTag != null ? parseOptionalString(row[mapping.errorTag]) : undefined,
      errorCost: mapping.errorCost != null ? Math.abs(parseNumber(row[mapping.errorCost])) : undefined,
      note: mapping.note != null ? parseOptionalString(row[mapping.note]) : undefined,
    })
  }
...
export function buildTradeMarkdown(date: string, records: TradeRecord[], realizedPnL?: number): string {
  ...
  lines.push("| 时间 | 代码 | 名称 | 方向 | 数量 | 价格 | 金额 | 手续费 | 印花税 | 过户费 | 模式 | 确定性 | 计划内 | 退出条件 | 错误标签 | 错误成本 | 备注 |")
  lines.push("|------|------|------|------|------|------|------|--------|--------|--------|------|--------|--------|----------|----------|----------|------|")

  for (const r of sorted) {
    const time = r.time || "—"
    const dir = r.direction === "buy" ? "买入" : "卖出"
    const planned = r.plannedTrade === true ? "计划内" : r.plannedTrade === false ? "计划外" : ""
    lines.push(
      `| ${formatMarkdownCell(time)} | ${formatMarkdownCell(r.code)} | ${formatMarkdownCell(r.name)} | ${dir} | ${r.quantity} | ${formatMoney(r.price)} | ${formatMoney(r.amount)} | ${formatMoney(r.fee)} | ${formatMoney(r.stampTax)} | ${formatMoney(r.transferFee)} | ${formatMarkdownCell(r.modeTag)} | ${formatMarkdownCell(r.certaintyTag)} | ${planned} | ${formatMarkdownCell(r.exitCondition)} | ${formatMarkdownCell(r.errorTag)} | ${r.errorCost != null ? formatMoney(r.errorCost) : ""} | ${formatMarkdownCell(r.note)} |`
    )
  }
