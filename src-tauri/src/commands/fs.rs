...
pub fn read_file(path: String) -> Result<String, String> {
    ...
    match ext.as_str() {
        ...
        _ => {
            // Prefer strict UTF-8, but fall back to lossy text for GBK/UTF-16-ish
            // markdown exports instead of incorrectly labeling them as binary.
            match fs::read_to_string(&path) {
                Ok(content) => Ok(content),
                Err(read_text_err) => match fs::read(&path) {
                    Ok(bytes) => Ok(String::from_utf8_lossy(&bytes).to_string()),
                    Err(read_bytes_err) => Err(format!(
                        "Failed to read file '{}': {}; fallback failed: {}",
                        path, read_text_err, read_bytes_err
                    )),
                },
            }
        }
