...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (["dry-run", "help", "force"].includes(key)) {
      args[key] = true
      continue
    }
...
function printHelp() {
  console.log(`Usage:
  npm run brain:push -- --project C:\\wiki\\73神话
  npm run brain:push -- --project C:\\wiki\\73神话 --dry-run
`)
}
...
function main() {
  ...
  const reportPath = path.resolve(args["report-path"] ?? path.join(projectPath, ".llm-wiki/learning-layers/latest-learning-layers.json"))
  const report = readJsonMaybe(reportPath)
  if (!report) throw new Error(`Missing learning layers report: ${reportPath}`)

  const pushEligible = Array.isArray(report.pushEligibleItems) ? report.pushEligibleItems : []
  ...
  const scriptPath = path.join(managerPath, "scripts", "send-brain-alerts.mjs")
  ...
  if (args.force) invokeArgs.push("--force")
